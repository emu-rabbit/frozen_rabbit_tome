...
import { calculateFoodBonus, getGatheringFood } from '../services/foodData';
...
export function calculateProfileEffectiveMaxGp(stats: PlayerStats, food: FoodSelection): number {
  const normalizedStats = normalizeStats(stats);
  const normalizedFood = cloneFood(food);
  const foodBonus = calculateFoodBonus(
    normalizedStats,
    getGatheringFood(normalizedFood.foodId),
    normalizedFood.quality
  );

  return clampIntegerInput(
    normalizedStats.gp + foodBonus.gp,
...
function normalizeProfile(profile: Partial<GearStatProfile>, fallbackJob: GatheringJob = 'miner'): GearStatProfile {
  ...
  const food = cloneFood(profile.food ?? NO_FOOD);
  ...
  return {
    ...
    gathering: stats.gathering,
    perception: stats.perception,
    currentGp: clampIntegerInput(profile.currentGp, PLAYER_INPUT_LIMITS.gp.min, effectiveMaxGp, effectiveMaxGp),
    maxGp: stats.gp,
    food,
